/**
 * @file webapp_handlers.c
 * @brief WebApp UI and Configuration API Handlers
 * 
 * HTTP request handlers for embedded webapp and configuration APIs:
 * - Static file serving (HTML/CSS/JS from embedded assets)
 ...
 * It registers handlers with the HTTP server core.
 */
...
esp_err_t webapp_handle_factory_reset(httpd_req_t *req);
esp_err_t webapp_handle_wifi_post(httpd_req_t *req);
esp_err_t webapp_handle_wifi_clear(httpd_req_t *req);
esp_err_t webapp_handle_ota_upload(httpd_req_t *req);
...
esp_err_t webapp_handle_api_status(httpd_req_t *req) {
    const char *mode_str = (s_mode == WEBAPP_MODE_CAPTIVE_PORTAL) ? "portal" : "normal";
...
esp_err_t webapp_handlers_register(httpd_handle_t server) {
    ...
    ESP_LOGI(TAG, "Registering webapp handlers...");
    ...
    static const httpd_uri_t api_status = {
        .uri = "/api/status",
        .method = HTTP_GET,
        .handler = webapp_handle_api_status,
        .user_ctx = NULL,
    };
    ...
    static const httpd_uri_t api_scan = {
        .uri = "/api/scan",
        .method = HTTP_GET,
        .handler = webapp_handle_api_scan,
        .user_ctx = NULL,
    };
    ...
    static const httpd_uri_t wifi_post = {
        .uri = "/wifi",
        .method = HTTP_POST,
        .handler = webapp_handle_wifi_post,
        .user_ctx = NULL,
    };
    ...
    static const httpd_uri_t wifi_clear = {
        .uri = "/wifi/clear",
        .method = HTTP_POST,
        .handler = webapp_handle_wifi_clear,
        .user_ctx = NULL,
    };
    ...
    static const httpd_uri_t factory_reset = {
        .uri = "/factory-reset",
        .method = HTTP_POST,
        .handler = webapp_handle_factory_reset,
        .user_ctx = NULL,
    };
    ...
    static const httpd_uri_t ota_upload = {
        .uri = "/ota/upload",
        .method = HTTP_POST,
        .handler = webapp_handle_ota_upload,
        .user_ctx = NULL,
    };
    ...
    ret = httpd_register_uri_handler(server, &api_status);
    if (ret == ESP_OK) registered_count++;
    
    ret = httpd_register_uri_handler(server, &api_scan);
    if (ret == ESP_OK) registered_count++;
    
    ret = httpd_register_uri_handler(server, &device_get);
    if (ret == ESP_OK) registered_count++;
    
    ret = httpd_register_uri_handler(server, &device_post);
    if (ret == ESP_OK) registered_count++;
    
    ret = httpd_register_uri_handler(server, &wifi_post);
    if (ret == ESP_OK) registered_count++;
    
    ret = httpd_register_uri_handler(server, &wifi_clear);
    if (ret == ESP_OK) registered_count++;
    
    ret = httpd_register_uri_handler(server, &factory_reset);
    if (ret == ESP_OK) registered_count++;
    ...
    ret = httpd_register_uri_handler(server, &style_css);
    if (ret == ESP_OK) registered_count++;
    
    ret = httpd_register_uri_handler(server, &app_js);
    if (ret == ESP_OK) registered_count++;
    
    ret = httpd_register_uri_handler(server, &wifi_alias);
    if (ret == ESP_OK) registered_count++;

    // In portal mode, redirect all unknown paths to /.
    ret = httpd_register_err_handler(server, HTTPD_404_NOT_FOUND, webapp_handle_404_redirect);
    if (ret == ESP_OK) registered_count++;

    ESP_LOGI(TAG, "Registered %d webapp handlers", registered_count);
...
void webapp_handlers_set_mode(webapp_mode_t mode) {
    const webapp_mode_t prev = s_mode;
    s_mode = mode;
...
// ============================================================================
// HANDLER FUNCTIONS (renamed from webapp_handle_* to webapp_handle_*)
// ============================================================================

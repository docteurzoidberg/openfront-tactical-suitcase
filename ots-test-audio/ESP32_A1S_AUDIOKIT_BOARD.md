...
- **MCLK must be on GPIO 0** → Essential for ES8388 operation
- Incorrect BCK pin causes I2S underflow errors
- Missing MCLK causes codec to not produce audio
...
**Symptoms**: Codec initializes, I2S runs, but no sound  
**Root Cause**: Missing MCLK or wrong BCK pin  
**Solution**:
- Verify MCLK on GPIO 0 (not optional!)
- Verify BCK on GPIO 27 (not GPIO 5)
- Check with logic analyzer if available
...
```
I (xxx) ES8388: ES8388 codec initialized successfully
I (xxx) ES8388: Power amplifier enabled (GPIO 21)
I (xxx) I2S: MCLK: GPIO 0
I (xxx) I2S: BCK:  GPIO 27
I (xxx) ES8388: Volume set to 100% (DACCONTROL4/5 reg=0x00)
I (xxx) ES8388: DAC started and unmuted (with soft-ramp)
```

**Success Criteria**:
- ✅ Audio plays at full volume
- ✅ No I2S errors in serial logs
- ✅ Minimal startup click (soft-ramp reduces it)
- ✅ Clean shutdown (no pops)

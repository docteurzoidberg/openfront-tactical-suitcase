 ...
 * 
 * Unified console interface for WAV playback and mixer testing
 * Uses ESP Console API for proper command handling
 */
...
#define TONE_START_DELAY_MS      100
...
static int cmd_play1(int argc, char **argv) {
    return play_wav_file("track1.wav", "sound 1");
}

static int cmd_play2(int argc, char **argv) {
    return play_wav_file("track2.wav", "sound 2");
}
...
// Helper function for tone playback
static int play_embedded_tone(const uint8_t *tone_data, const char *description)
{
    ESP_LOGI(TAG, "Playing %s...", description);
    
    wav_info_t wav;
    if (wav_parse_header_from_memory(tone_data, &wav) == ESP_OK) {
        audio_source_handle_t handle;
        esp_err_t ret = audio_mixer_create_source_from_memory(
            tone_data + wav.data_offset,
            wav.data_size,
            DEFAULT_TONE_VOLUME,
            false, // no loop
            false, // no interrupt
        ...
        if (ret == ESP_OK) {
            ESP_LOGI(TAG, "✓ %s started", description);
        } else {
...
// Test tones from embedded memory
static int cmd_tone1(int argc, char **argv) {
    return play_embedded_tone(tone_1s_440hz_wav, "tone 1 (1s, 440Hz)");
}

static int cmd_tone2(int argc, char **argv) {
    return play_embedded_tone(tone_2s_880hz_wav, "tone 2 (2s, 880Hz)");
}

static int cmd_tone3(int argc, char **argv) {
    return play_embedded_tone(tone_5s_220hz_wav, "tone 3 (5s, 220Hz)");
}
...
static int cmd_mix(int argc, char **argv)
{
    ...
    cmd_tone1(0, NULL);
    vTaskDelay(pdMS_TO_TICKS(TONE_START_DELAY_MS));
    cmd_tone2(0, NULL);
    vTaskDelay(pdMS_TO_TICKS(TONE_START_DELAY_MS));
    cmd_tone3(0, NULL);
...
{
    ESP_LOGI(TAG, "Playing built-in test tone...");
    audio_test_tone_generate();
    return 0;
}
...
static int cmd_info(int argc, char **argv)
{
    ...
    ESP_LOGI(TAG, "Tone 1: %d bytes (1s @ 440Hz)", tone_1s_440hz_wav_len);
    ESP_LOGI(TAG, "Tone 2: %d bytes (2s @ 880Hz)", tone_2s_880hz_wav_len);
    ESP_LOGI(TAG, "Tone 3: %d bytes (5s @ 220Hz)", tone_5s_220hz_wav_len);
    ESP_LOGI(TAG, "Total: %d bytes", 
             tone_1s_440hz_wav_len + tone_2s_880hz_wav_len + tone_5s_220hz_wav_len);
    return 0;
}
